...
class NeuralNet:
    ...
    def feed_forward( self ):
        ...
        #self.ch[ f'A{len( self.dims ) - 1}' ] = np.tanh( self.ch[ f'Z{len( self.dims ) - 1}' ] )
    ...
    def back_prop( self ):
        ...
        for i in np.arange( len( self.dims ) - 2, 0, -1 ):
            if (i == len( self.dims ) - 2):
                ...
                #dLoss_Z[i] = dLoss_A[i] * dtanh( self.ch[ f'Z{i+1}' ] )
    ...
    def gd( self, iter = 3000 ):
        ...
        plt.ylabel( 'Loss' )
        plt.xlabel( r'Iteration ($\times$10)' )
        plt.title( f"Learning rate = {str( self.lr )}" )
        plt.savefig( f"/Users/zhn11tau/Documents/Thesis_pics/nn_{self.lr_str}_varsig_soft_newtest1.pdf", format = 'pdf' )
        plt.show()
...
if __name__ == "__main__":
    ...
    nn = NeuralNet( x, y, 0.001 )
    nn.dims = [len(x), 10, 8, 7, 6, 5, 4, 3, 2, 1]
    nn.threshold = 0.7
    nn.gd( iter = 20000 )
    ...
    plot_cf( target, predicted, f'Accuracy: {100*np.sum( (pred_train == y)/x.shape[1] ):.2f}$\%$', save = f"/Users/zhn11tau/Documents/Thesis_pics/nn_{nn.lr_str}_varsig_soft_cf_newtest1.pdf" )
    ...
    if SAVE:
        nn.save_params( root = 'NN2/J1829+2456' )
